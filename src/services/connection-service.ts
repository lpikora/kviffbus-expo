...
export function searchConnections(
  ...
  const sameDayResults = connectionResultsForDate(
    group,
    minutesFromMidnight(departureDateTime),
    departureDateTime,
    appConfig,
  );
  const nextDay = nextDayAtMidnight(departureDateTime);
  const nextDayResults = connectionResultsForDate(
    group,
    0,
    nextDay,
    appConfig,
...
function exceptionsByStopId(exceptions: StopExceptionDto[]) {
...
function hasStopException(
  ...
  for (const exception of exceptions) {
    const fromDate = dateWithMinutes(
      dateFromKey(exception.fromDate),
      exception.fromTime,
    );
    const toDate = dateWithMinutes(
      dateFromKey(exception.toDate),
      exception.toTime,
    );
...
function excludeStopExceptions(
  connections: ConnectionResult[],
  exceptions: StopExceptionDto[],
  fromStopId: number,
  toStopId: number,
): ConnectionResult[] {
  const byStopId = exceptionsByStopId(exceptions);

  return connections.filter((connection) => {
    const { timeDeparture, timeArrival } = connection.departureArrivalTimes;
    const departureDateTime = dateWithMinutes(
      connection.departureDate,
      timeDeparture,
    );
    const arrivalDateTime = dateWithMinutes(
      connection.departureDate,
      timeArrival,
    );
    ...
    if (hasStopException(byStopId.get(toStopId), arrivalDateTime)) {
      return false;
    }

    if (hasStopException(byStopId.get(fromStopId), departureDateTime)) {
      return false;
    }
...
function connectionResultsForDate(
  group: ConnectionDto[],
  ...
  const fromIndex = lowerBound(group, minDepartureMinutes);
  ...
  return excludeOutsideOperations(
    toConnectionResults(uniqueByDeparture, departureDateTime),
    dateKey,
    appConfig,
  );
}

function lowerBound(connections: ConnectionDto[], minDepartureMinutes: number) {
  let lo = 0;
...
function excludeOutsideOperations(
  connections: ConnectionResult[],
  ...
  return connections.filter((connection) => {
    ...
    if (operationsStartDate && connection.departureDate < operationsStartDate) {
      return false;
    }
...
function dateWithMinutes(baseDate: Date, minutesFromMidnight: number) {
...
function nextDayAtMidnight(date: Date) {
